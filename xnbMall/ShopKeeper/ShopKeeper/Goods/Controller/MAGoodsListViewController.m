/**
 * MAGoodsListViewController.m 16/11/12
 *
...
@interface MAGoodsListViewController ()<MABrandBoardDelegate, UISearchResultsUpdating, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UISearchControllerDelegate> {
...
@implementation MAGoodsListViewController
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [self.view setFrame:_frame];//从search resultcontroller返回，self.view坐标上移了。。。。（如果result又有跳转）
}
...
- (void)createUI {
    ...
    CGFloat brandBtnH = 50.f;
    
    CGFloat collectionViewH = SCREEN_HEIGHT - 64 - brandBtnH;
    UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0.f, brandBtnH, SCREEN_WIDTH, collectionViewH) collectionViewLayout:flowLayout];
    ...
    _brandBoard = [[MABrandBoard alloc] initWithFrame:collectionView.frame];
    _brandBoard.delegate = self;
...
- (void)initBrandBtn {
    ...
    [_brandBtn setTitleColor:RGBGRAY(100.f) forState:UIControlStateNormal];
...
- (void)initSearchController {
    ...
    MAGoodsSearchResultViewController *vc = [MAGoodsSearchResultViewController new];
    vc.fatherController = self;
    
    UINavigationController *c1   = [[UINavigationController alloc] initWithRootViewController:vc];
    c1.navigationBar.barStyle = UIBarStyleBlackOpaque;
    c1.navigationBar.hidden = YES;
    
    _searchController = [[UISearchController alloc] initWithSearchResultsController:c1];
    ...
    _searchController.searchBar.barTintColor =  HEXCOLOR(0xEE2C2Cff);
    _searchController.searchBar.placeholder = _categoryName;
    _searchController.searchBar.tintColor = RGBGRAY(240.f);
    _searchController.delegate = self;

    self.navigationItem.titleView = _searchController.searchBar;
    if (_keywords != nil) {
        _searchController.searchBar.text = _keywords;
    }
