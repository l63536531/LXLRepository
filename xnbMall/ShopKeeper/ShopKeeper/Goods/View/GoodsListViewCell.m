...
-(void)makeCellLeftView{
    ...
    [label_5 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(label_4.mas_bottom).offset(2);
        make.left.equalTo(_bv_1).offset(2);
        make.right.equalTo(_bv_1.mas_right).offset(-2);
        make.height.mas_equalTo(@(20));
    }];
    ...
    label_3.text=[NSString stringWithFormat:@"进货价 : ¥%.2f",costPrice];
    label_3.textColor = HEXCOLOR(0xEE2C2Cff);
    label_3.font = [UIFont systemFontOfSize:14];

    
    //label_4.backgroundColor=HEXCOLOR(0x71C671ff);
    float retailPrice  = [goodsPrice[@"retailPrice"] floatValue];
    NSDictionary *attribtDic = @{NSStrikethroughStyleAttributeName: [NSNumber numberWithInteger:NSUnderlineStyleSingle]};
    NSMutableAttributedString *attribtStr = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"建议售价：¥ %.2f",retailPrice] attributes:attribtDic];
    label_4.attributedText = attribtStr;
