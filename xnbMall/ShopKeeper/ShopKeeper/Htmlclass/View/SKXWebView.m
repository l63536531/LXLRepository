...
@interface SKXWebView ()<UIAlertViewDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        // 注入JS对象名称AppModel，当JS通过GoBack来调用时，
        ...
        self.backgroundColor = KBackColor(@"#ec584c");
        ...
        [self.progressView setFrame:CGRectMake(0,20.f, self.frame.size.width, self.progressView.frame.size.height)];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if(webView == self.wkWebView) {
        ...
        else if([[UIApplication sharedApplication] canOpenURL:URL]) { //跳转加载
...
#pragma mark - WKScriptMessageHandler js交换配置
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"GoBack"]) {
        // 打印所传过来的参数，只支持NSNumber, NSString, NSDate, NSArray,
        // NSDictionary, and NSNull类型
     NSLog(@"WKScriptMessageHandler=====%@", message.name);
        
        //发通知
        [[NSNotificationCenter defaultCenter]postNotificationName:WKScriptMessageDidChangedNotification object:self];
    }
