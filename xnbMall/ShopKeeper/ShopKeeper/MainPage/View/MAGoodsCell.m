...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self != nil) {
        ...
        _sellCountLabel = [[JKLabel alloc] initWithFrame:CGRectMake(4.f, _priceLabel.maxY + 0.f, width - 8.f, labelH)];
        _sellCountLabel.font = FONT_HEL(12.f);
        _sellCountLabel.textColor = RGBGRAY(200.f);
...
- (void)setSellCount:(CGFloat)sellCount {
    _sellCountLabel.text = [NSString stringWithFormat:@"已售：%.0f",sellCount];
}
...
- (void)setIsSellOut:(BOOL)isSellOut {
    ...
    if (_isSellOut) {
        ...
        _priceLabel.text = [NSString stringWithFormat:@"售价：￥%.2f",_price];
    }
}
