...
- (void)initView {
    ...
    self.overlayView.backgroundColor = KFontColor(@"#646464");
...
- (void)initControlBtn {
    ...
    cancelBtn.backgroundColor = KFontColor(@"#646464");
    ...
    confirmBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 100.0f, self.view.frame.size.height - 50.0f, 100, 50)];
    confirmBtn.backgroundColor = KFontColor(@"#646464");
    confirmBtn.titleLabel.textColor = [UIColor whiteColor];
    [confirmBtn setTitle:@"剪辑" forState:UIControlStateNormal];
...
- (void)cancel:(id)sender {
    ...
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(VPImageCropperDelegate)]) {
        [self.delegate imageCropperDidCancel:self];//图片取消
...
- (void)confirm:(id)sender {
        ...
        if (self.delegate && [self.delegate conformsToProtocol:@protocol(VPImageCropperDelegate)]) {
                ...
                [self.delegate imageCropper:self didFinished:[self getSubImage]];//图片确定
...
/*
 ...
 {
 UIImage* image2;
 UIImageWriteToSavedPhotosAlbum(image2, self, @selector(image:didFinishSavingWithError:contextInfo:), NULL);
...
/* -(void)rotate:(UIRotationGestureRecognizer *)rotate {
    ...
    if(rotate.state == UIGestureRecognizerStateEnded) {
     ...
     CGFloat rotation = 0.0 - (_lastRotation - rotate.rotation);
     
     CGAffineTransform currentTransform = self.showImgView.transform;
     CGAffineTransform newTransform = CGAffineTransformRotate(currentTransform,rotation);
     
     [self.showImgView setTransform:newTransform];
    ...
    [UIView animateWithDuration:BOUNDCE_DURATION animations:^{
     ...
     //self.showImgView.frame = [self handleBorderOverflow:self.showImgView.frame];
...
-(UIImage *)getSubImage{
    ...
    CGImageRef subImageRef = CGImageCreateWithImageInRect(imageRef, myImageRect);//剪切区域
    CGSize size;
    ...
    // [self saveImageToPhotos:smallImage];
//    NSLog(@"%f  dsfg  %f",smallImage.size.width,smallImage.size.height);
    return smallImage;
}
