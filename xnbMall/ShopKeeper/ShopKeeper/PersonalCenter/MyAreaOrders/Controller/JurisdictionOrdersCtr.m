...
@interface JurisdictionOrdersCtr ()

@end

@implementation JurisdictionOrdersCtr
...
-(void)rightBarbutton{
    ...
    JurisdictionSearchCtr* vc = [[JurisdictionSearchCtr alloc] init];
...
-(void)getData{
    ...
    [[TransDataProxyCenter shareController] queryYtorders:getindex  searchKey:@"" pageIndex:@"0" block:^(NSDictionary *dic, NSError *error) {
        if (dic) {
...
-(void)footerRereshing{
    ...
    [[TransDataProxyCenter shareController] queryYtorders:getindex  searchKey:@"" pageIndex:[NSString stringWithFormat:@"%ld",pageindex] block:^(NSDictionary *dic, NSError *error) {
        if (dic) {
            NSNumber* code = dic[@"code"];
            NSString* msg = [error localizedDescription];
            if ([code intValue]  == 200) {
