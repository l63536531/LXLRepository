...
#import "MyServiceSearchEndCtr.h"
...
@interface MyServiceSearchEndCtr ()

@end

@implementation MyServiceSearchEndCtr
...
-(void)getData{
    ...
    [[TransDataProxyCenter shareController] querymyserviceorders:(int)_getindid  searchKey:_getkeyString pageIndex:@"0" block:^(NSDictionary *dic, NSError *error) {
        if (dic) {
...
-(void)footerRereshing{
    ...
    [[TransDataProxyCenter shareController] querymyserviceorders:(int)_getindid  searchKey:_getkeyString pageIndex:[NSString stringWithFormat:@"%ld",pageindex] block:^(NSDictionary *dic, NSError *error) {
        if (dic) {
            NSNumber* code = dic[@"code"];
            NSString* msg = [error localizedDescription];
            if ([code intValue]  == 200) {
