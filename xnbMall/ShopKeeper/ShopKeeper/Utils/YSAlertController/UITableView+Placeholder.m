...
-(void)setLoading:(BOOL)loading
{
    ...
    [self reloadEmptyDataSet]; //sorllviwe的代理方法
...
{
    objc_setAssociatedObject(self, &dataVerticalOffsetKey,@(dataVerticalOffset),OBJC_ASSOCIATION_RETAIN);// 如果是对象，用RETAIN>注意
}
...
- (UIImage *)imageForEmptyDataSet:(UIScrollView *)scrollView
{
    if (self.loading) {
        ...
        NSString *imageName = @"quan5";//@"defaultHeadIcon";
...
- (NSAttributedString *)buttonTitleForEmptyDataSet:(UIScrollView *)scrollView forState:(UIControlState)state
{
    if (self.loading) {
        ...
        return [[NSAttributedString alloc] initWithString:self.buttonText ? self.buttonText : @"点我再次刷新" attributes:attributes];
    }
}
